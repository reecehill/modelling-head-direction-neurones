...
class NeuronalPopulation:
    def __init__(self):
        ...
        if(p.generateWeightsMethod == 'independentNeurone'):
            ...
            self.neurones = np.array([Neurone(theta_0=0) for theta in p.thetaSeries])
            # TODO: This function, np.roll, has not been tested here. Check output if using templateNeurone.
            self.neurones = [np.roll(neurone.evenWeights, neuroneIndex) for neuroneIndex, neurone in enumerate(self.neurones)]
